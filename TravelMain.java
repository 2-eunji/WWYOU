		...
		while (run) {
			...
			int menu=sc.nextInt();
				...
				case 1: //여행자 후기
					...
					case 1:
						System.out.println("명소 조회 !");
						System.out.println("명소 >> ");
						String attraction = sc.next();

						boolean check = sql.attraction(attraction);

						if (check) {
							System.out.println("후기 입력이 가능합니다.");
							System.out.print("고유 번호 입력>>");
							String r_privatenum=sc.next();
							review.setR_privatenum(r_privatenum);			
							System.out.print("관광지 입력 >> ");
							String r_attraction = sc.next();
							review.setR_attraction(r_attraction);
							System.out.print("후기 입력 >> ");
							String review1 = sc.next();
							review.setReview(review1);
							sql.insertReview(review);
						}else {
				...
				case 5: // 나라명 검색
					...
					if (check) {
						...
						System.out.println("등록되지 않은 나라입니다. 등록을 원하시면 고객센터로 문의해주세요.");
					}
					break;
				...
				case 7: //관리자모드
						...
						if (password.equals("양준희")) {
							...
							case 1: // 여행지 입력
								...
								System.out.println();
								System.out.println(" 1. 수도권     │ 2. 강원도     │ 3. 전라도       │4. 충청도       │5. 제주도       │6. 경상도 ");
					...
					sql.conClose();
					System.out.println("데이터베이스 접속을 해제합니다.");
					System.out.println();
					run = false;
					break;
				default:
