...
// Case1-1.후기 작성
	...
	public void insertReview(Review review) { // 여행지입력
		String sql = "INSERT INTO REVIEW VALUES  (?,?, ?)";
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, review.getR_privatenum());
			pstmt.setString(2, review.getR_attraction());
...
//	case1-2.후기 보기
	...
	public void reviewJoin(String pnum) {
		 try {
			   String sql = "SELECT * FROM REVIEW WHERE R_ATTRACTION = ?";
			   pstmt = con.prepareStatement(sql);
			   pstmt.setString(1, pnum);
			   rs = pstmt.executeQuery();
			   
			   while (rs.next()) {
			    System.out.println("관광지 : " + rs.getString(2));
			    System.out.println("후기 : " + rs.getString(3));
			    System.out.println();
			   }   
...
public boolean attraction(String attraction) {
	...
	public void insertAttraction(TravelList travelList) { // 여행지입력
		...
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, travelList.getPrivatenum()); // PRAVATENUM
			pstmt.setInt(2, travelList.getCountryNum()); // 나라번호
...
// Case3. 랜덤 여행지 추천
	...
	public void recommendRandom(int rNum) {
		if (rNum == 1) {
			try {
				stmt = con.createStatement();
				String sql = "SELECT * FROM (SELECT * FROM TRAVELLIST WHERE COUNTRYNUM != 41  ORDER BY DBMS_RANDOM.VALUE) WHERE ROWNUM < 4";
				rs = stmt.executeQuery(sql);
				System.out.println();
				...
				while (rs.next()) {
					...
					System.out.println("명소 : " + rs.getString(5));
					System.out.println();
				}
			} catch (Exception se) {
				se.printStackTrace();
	...
	public void Taste2(int sNum, String recommendNum1, String recommendNum2, String recommendNum3) {
		if (sNum == 1) {
			...
			try {
				...
				System.out.println();
				while (rs.next()) {
					System.out.println("지역 :" + rs.getString(3));
					System.out.println("도시 :" + rs.getString(4));
					System.out.println("명소 :" + rs.getString(5));
					System.out.println();
				}
